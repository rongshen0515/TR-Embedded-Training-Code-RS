...
int main(){
    //SETUP CODE HERE
    DJIMotor yawMotor(1, CANHandler::CANBUS_2, GM6020, "Yaw");
    DJIMotor pitchMotor(2, CANHandler::CANBUS_2, GM6020, "Pitch");

    int pitchTarget = 0;

    constexpr int JOY_MAX = 660;          
    constexpr int MAX_YAW_SPEED = 8000;   
    constexpr int PITCH_STEP_PER_TICK = 30; 
    constexpr int PITCH_MIN = -4000;     
    constexpr int PITCH_MAX =  4000;

    while(true){ //main loop
        remoteRead(false);

        const auto rsw = remote.rightSwitch();
        const int rx = remote.rightX();   
        const int ry = remote.rightY();   

        const bool enabled = (rsw == Remote::SwitchState::UP);

        if(enabled) {

            int yawCmd = (rx * MAX_YAW_SPEED) / JOY_MAX;
            yawMotor.setSpeed(yawCmd);

            pitchTarget += (-ry * PITCH_STEP_PER_TICK) / JOY_MAX;

            if(pitchTarget < PITCH_MIN) pitchTarget = PITCH_MIN;
            if(pitchTarget > PITCH_MAX) pitchTarget = PITCH_MAX;
